...
For tool: `veil`
Target: `/Users/zac/Source/cmdrvl/veil`
...
This workspace lives **inside the target repo** at `/Users/zac/Source/cmdrvl/veil/agent_ergonomics_audit/` and is committed alongside the code on the target's current branch (typically `main`). The phase-loop scripts live in the **skill repo**, not in this workspace. From the skill repo's root (or with absolute paths), run:

1. `<SKILL>/scripts/discover-cli.sh /Users/zac/Source/cmdrvl/veil` to confirm the binary still exists.
2. `<SKILL>/scripts/validate_pass.sh /Users/zac/Source/cmdrvl/veil/agent_ergonomics_audit` to check artifact integrity.
